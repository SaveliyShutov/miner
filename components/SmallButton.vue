...
const triggerConfetti = () => {
  useConfetti({ particleCount: 100, spread: 70, origin: { y: 0.7 } })
}
...
let claimed = computed(() => {
  if (userStore.user?.claimedTasks) {
    let _claimed = userStore.user.claimedTasks.includes(taskId)
    clicked.value = _claimed
    return _claimed
  }
  return false
})
...
<template>
  <button
    ...
    @click=";(clicked = true)"
    type="button"
    class="text-white bg-zinc-700 font-medium rounded-full text-sm px-5 py-2.5 text-center"
  >
    Start
  </button>
  <button
    v-if="!claimed && clicked == true"
    @click="claim"
    type="button"
  ...
  <div v-if="claimed && clicked == true" class="text-zinc-600 text-sm px-5 py-2.5 text-center">
